...
export default function TimeEntryGrid({ clientId, dateFrom, dateTo }: TimeEntryGridProps) {
  ...
  function renderEntryCard(entry: TimeEntry, dateStr: string, compact: boolean) {
    ...
    if (!compact) {
      ...
      return (
        <button
          ...
          className="w-full text-left rounded-md border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer flex items-center justify-between gap-4"
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <span className="text-gray-800 dark:text-gray-200">{entry.jobType?.name || 'Unknown'}</span>
            {entry.tech && <span className="text-gray-400 dark:text-gray-500 text-xs">{entry.tech.displayName}</span>}
          </div>
          <div className="flex items-center gap-3 flex-shrink-0">
            <span className="text-gray-500 dark:text-gray-400">{entry.hours}h</span>
            {entry.total && <span className="text-gray-600 dark:text-gray-400 font-medium">{formatCurrency(parseFloat(entry.total))}</span>}
            <div className="flex gap-1">
              {(entry.invoicePaid || entry.isPaid) ? (
                <span className="px-1 py-0 rounded text-[10px] bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400">Paid</span>
              ) : entry.isBilled ? (
                <span className="px-1 py-0 rounded text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">Billed</span>
              ) : null}
            </div>
            {admin && (
              <span onClick={(e) => toggleSelect(entry.id, e)}>
                {isSelected ? <CheckSquare className="w-4 h-4 text-blue-500" /> : <Square className="w-4 h-4 text-gray-300 dark:text-gray-600 hover:text-gray-500 dark:hover:text-gray-400" />}
              </span>
            )}
          </div>
        </button>
    ...
    return (
      <button
        key={entry.id}
        onClick={() => handleCellClick(dateStr, entry)}
        className={`w-full text-left rounded-md border px-2 py-1 text-xs hover:border-gray-300 dark:hover:border-gray-600 transition-colors cursor-pointer ${
          isSelected ? 'border-blue-400 dark:border-blue-600 bg-blue-50 dark:bg-blue-900/30' : 'border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'
        }`}
      >
        {showClientName && (
        ...
        <div className="flex items-center justify-between gap-1">
          <div className="font-medium text-gray-800 dark:text-gray-200 truncate flex-1">
            {entry.jobType?.name || 'Unknown'}
          </div>
          <div className="flex items-center gap-1 flex-shrink-0">
            {entry.tech && <span className="text-gray-400 dark:text-gray-500 truncate max-w-[60px]">{entry.tech.displayName}</span>}
            {admin && (
              <span onClick={(e) => toggleSelect(entry.id, e)}>
                {isSelected ? <CheckSquare className="w-3 h-3 text-blue-500" /> : <Square className="w-3 h-3 text-gray-300 dark:text-gray-600 hover:text-gray-500 dark:hover:text-gray-400" />}
              </span>
            )}
          </div>
        </div>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            {(entry.invoicePaid || entry.isPaid) ? (
              <span className="px-1 py-0 rounded text-[10px] bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400">P</span>
            ) : entry.isBilled ? (
              <span className="px-1 py-0 rounded text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">B</span>
            ) : null}
            {entry.total && <span className="text-gray-600 dark:text-gray-400 font-medium">{formatCurrency(parseFloat(entry.total))}</span>}
          </div>
        </div>
  ...
  function renderWeekRow(weekDates: Date[], weekIndex: number) {
    ...
    return (
      <div key={weekIndex} className={`${weekIndex > 0 ? 'border-t-2 border-gray-300 dark:border-gray-600' : ''}`}>
        <div className="grid grid-cols-7 divide-x divide-gray-200 dark:divide-gray-700">
          {weekDates.map((date) => {
            ...
            return (
              <div
                key={dateStr}
                className={`min-h-[160px] flex flex-col ${isToday(date) ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''}`}
              >
                {/* Day header */}
                <div
                  className={`px-2 py-1.5 border-b border-gray-200 dark:border-gray-700 text-center ${
                    isToday(date) ? 'bg-blue-100/60 dark:bg-blue-900/40' : isCurrentWeek ? 'bg-gray-50 dark:bg-gray-900' : 'bg-gray-100/50 dark:bg-gray-800/50'
                  }`}
                >
                  <div className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{SHORT_DAY_NAMES[date.getDay()]}</div>
                  <div className={`text-sm font-semibold ${isToday(date) ? 'text-blue-700 dark:text-blue-400' : 'text-gray-800 dark:text-gray-200'}`}>
                ...
                <div className="border-t border-gray-100 dark:border-gray-700 px-1.5 py-1">
                  {clientId && (
                    <button
                      onClick={() => handleAddClick(dateStr)}
                      className="w-full flex items-center justify-center gap-1 text-xs text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 py-0.5 rounded hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                    >
                  ...
                  {dayHours > 0 && (
                    <div className="text-center text-[11px] mt-0.5 pt-0.5 border-t border-gray-100 dark:border-gray-700">
                      <span className="text-gray-500 dark:text-gray-400">{dayHours}h</span>
                      <span className="text-gray-400 dark:text-gray-500 mx-0.5">|</span>
                      <span className="text-gray-600 dark:text-gray-400 font-medium">{formatCurrency(dayAmount)}</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        {/* Week total row */}
        <div className={`border-t border-gray-200 dark:border-gray-700 px-4 py-1.5 flex items-center justify-between ${isCurrentWeek ? 'bg-blue-50/30 dark:bg-blue-900/20' : 'bg-gray-50/50 dark:bg-gray-800/50'}`}>
          <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Week Total</span>
          <div className="text-xs">
            <span className="font-semibold text-gray-700 dark:text-gray-300">{weekHours}h</span>
            <span className="text-gray-400 dark:text-gray-500 mx-1.5">|</span>
            <span className="font-semibold text-gray-700 dark:text-gray-300">{formatCurrency(weekAmount)}</span>
          </div>
        </div>
      </div>
    );
      ...
      {useListView ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          {dates.map((date) => {
            ...
            return (
              <div key={dateStr} className={`border-b border-gray-200 dark:border-gray-700 last:border-b-0 ${isToday(date) ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''}`}>
                <div className={`px-4 py-2 flex items-center justify-between ${isToday(date) ? 'bg-blue-100/60 dark:bg-blue-900/40' : 'bg-gray-50 dark:bg-gray-900'}`}>
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                    {SHORT_DAY_NAMES[date.getDay()]} {date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                  </span>
                  <span className="text-xs text-gray-500 dark:text-gray-400">{dayHours}h | {formatCurrency(dayAmount)}</span>
                </div>
          ...
          {entries.length === 0 && (
            <div className="p-8 text-center text-gray-500 dark:text-gray-400">No entries in this date range.</div>
          )}
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 px-4 py-3 flex items-center justify-between">
            ...
            <div className="text-sm">
              <span className="font-semibold text-gray-800 dark:text-gray-200">{totalHours}h</span>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          {/* Grand total */}
          {weeks.length > 1 && (
            <div className="border-t-2 border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 px-4 py-3 flex items-center justify-between">
              <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Total ({weeks.length} weeks)</span>
              <div className="text-sm">
                <span className="font-semibold text-gray-800 dark:text-gray-200">{totalHours}h</span>
                <span className="text-gray-400 dark:text-gray-500 mx-2">|</span>
                <span className="font-semibold text-gray-800 dark:text-gray-200">{formatCurrency(totalAmount)}</span>
